...
stat <- numeric(n_sims) # cloumn to store the estimates of pi
for (i in 1:n_sims) {
  x <- runif(n, min = -1, max = 1)
  y <- runif(n, min = -1, max = 1)
  r <- sqrt(x^2 + y^2)
  in_circle <- r <= 1
  stat[i] <- 4 * sum(in_circle) / n
}
```
...
mean_stat <- mean(stat)
sd_stat <- sd(stat)
z_alpha <- qnorm(1 - 0.01 / 2)
ci_lower <- mean_stat - z_alpha * sd_stat / sqrt(n_sims)
ci_upper <- mean_stat + z_alpha * sd_stat / sqrt(n_sims)
...
- Does your interval include the true value of $\pi$?
  - Yes! The true value is in range.
- What confidence level did you choose?
  - 99% confidance
- Was your sample size $n$ large enough? Why do you say that?
  - Yes. I was able to get the true value of pi to fall in between the
    interval with a high confidence value.
